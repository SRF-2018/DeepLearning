...
b=tf.Variable(tf.random_normal([1,classes]))

hypothesis=tf.nn.softmax(tf.matmul(X,W)+b)

r=1e-3
cost=tf.reduce_mean(-tf.reduce_sum(Y*tf.log(hypothesis),1))+r*tf.reduce_sum(tf.square(W))
...
with tf.Session() as sess:
    ...
    for step in range(20001):
        ...
        if step%2000==0:
            print("cost : ", sess.run(tf.reduce_mean(ncost)))
    ac=0
